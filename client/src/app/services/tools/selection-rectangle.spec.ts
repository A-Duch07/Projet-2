...
describe('SelectionRectangleService', () => {
    ...
    it(' onMouseUp should call changePassedDImensions with offsetPositionSquare if squareMode is selected', () => {
        ...
        service.onMouseUp(mouseEvent);
        expect(resizeServiceSpy.newSelection).toHaveBeenCalledWith({ x: 20, y: 20 }, { x: 5, y: 5 }, 1, []);
    });
    ...
    it(' onMouseMove should not call drawLine if an image is not selected and mouseDownCoord is outside the canvas', () => {
        service.mouseDown = true;
        service.mouseDownCoord = { x: 0, y: 0 };
        service['drawingService'].imageSelected = false;
